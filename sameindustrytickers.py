...
def get_sample(ticker, vol_samplesize, vol_window, decrease_percentage):
    ...
    for sit in industrytickers:
        try:
            ...
            for X in list(range(-100,50)):
                df['arounddecrease'] = np.where(df.perc_change.shift(X)<decrease_percentage, 1, df.arounddecrease)
    ...
    for ticker in sample:
        df = df_industrytickers[(df_industrytickers.ticker == ticker) | (df_industrytickers.arounddecrease == 1)]
...
#%%
#df_sample = get_sample('BGI', vol_samplesize=2, vol_window=3, decrease_percentage=0.8)
