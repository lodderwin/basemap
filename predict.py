...
X = df[['open_sc','high_sc','low_sc','close_sc','volume_sc',
        'year_sc',#'dom_sc','weekday_sc','month_sc',
        ...
        'high_low_min5_sc','open_cagr_sc','close_cagr_sc','code_en']]
y = df['regressor_y']
...
plots.dualHist(rf.predict(X_test) - y_test, 
               rf.predict(X_train) - y_train,
               np.sqrt(np.mean((rf.predict(X_test) - y_test) ** 2)).astype(int), 
               np.sqrt(np.mean((rf.predict(X_train) - y_train) ** 2)).astype(int), 
               'test', 
               'train', 
